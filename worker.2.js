    ...
    db.any(selectQuery, [true])
      .then((data) => {
        ...
        if (settings.sco != null && settings.sco === 'false'){
          ...
          console.log(settings.tokenName);
      ...
      // create
      .then((settings) => {
          if (settings.sco != null && settings.sco === "true"){
            console.log('***Inside create', settings.guid);
            setNewStage(settings, 'create');
          } else {
      ...
      .then((settings) => {
        if (settings.sco != null && settings.sco === 'true'){
          deploymentStage(settings);
        }
        return settings;
      })
      .then((settings) => {
        if (settings.sco != null && settings.sco === 'true'){
          executeScript(settings, settings.createScript);
        }
        return settings;
      })
      .then((settings) => {
