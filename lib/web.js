...
module.exports = function (app) {
  ...
  app.get('/', (req, res) => {
    ...
    postgresHelper.getDeploymentCount().then((deploymentCountResult) => {
      ...
      const sco = req.query.sco;
      console.debug('app.get root template=' + template);
      console.debug('app.get sco=' + sco);
  ...
  app.get('/deploy', (req, res) => {
    ...
    const sco = req.query.sco;
    console.debug('app.get deploy sco=' + sco);
    console.debug('app.get deploy template=' + template);
        ...
        conn.tooling.query("SELECT DurableId, SettingValue FROM OrganizationSettingsDetail WHERE SettingName = 'ScratchOrgManagementPref'", (devHubErr, result) => {
          ...
          if (result.size > 0) {
            ...
            if (devHubEnabled === true) {
              if (template) {
                return res.redirect(`/deploy?sco=true&template=${template}`);
              } else {
                return res.redirect('/choose');
              }
            } else {
              return res.redirect(`/deploy?sco=false&template=${template}`); 
