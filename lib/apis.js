...
module.exports = function (router) {
  ...
  router.post('/deploy', (req, res) => {
    ...
    console.debug("*** API deploy called!" + "${settings.guid}" + ' ' + "${settings.sco}");

    console.debug("*** API deploy access_token");
    console.debug(req.cookies.access_token);
    ...
    postgresHelper.insertDeployment(insertQuery).then(() => {
      console.debug("*** API postgresHelper.insertDeployment done!");
      res.json({
