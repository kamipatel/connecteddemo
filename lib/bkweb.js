...
module.exports = function(app) {
  ...
  app.get("/testjwt", (req, res) => {
    console.debug("testjwt called");
    var privateKey = fs.readFileSync(__dirname + "/server.key", "utf8");
    console.debug("testjwt privateKey=" + privateKey);

    var clientId =
      "3MVG9KsVczVNcM8xbwLLOjukOoi0IuFqWALggssV2JFGCecWm3xqLJt4nDEsVH6_KvGkDFhc42q221S0gYfGV";

    var instanceUrl = "https://contestclient-dev-ed.my.salesforce.com";
    var username = "flow@lab.app";
...
*/
  ...
  app.post("/SendDataByJwt", (req, res) => {
    console.debug("SendDataByJwt called, req=" + JSON.stringify(req.body));

    //var privateKey = fs.readFileSync('./server.key', 'utf8');
    var privateKey = fs.readFileSync(__dirname + "/server.key", "utf8");
    //    console.debug("SendDataByJwt privateKey=" + privateKey);
    ...
    docClient.query(qparams, function(err, data) {
      if (err) {
        console.debug("SendDataByJwt dynamodb query error=" + err);
        res.send({
          success: false,
          message: "Error: aws dynamodb read error"
        });
      } else {
        console.debug("SendDataByJwt dynamodb query success, data=" + data);
        console.log("data", data);
        var o = data.Items[0];

        console.debug("Before jwtdata, privateKey=" + privateKey);
        console.debug("Before jwtdata, consumerKey=" + consumerKey);
        console.debug("Before jwtdata, o.username=" + o.username);

        try {
          jwtflow.getToken(consumerKey, privateKey, o.username, function(
            err,
            jwtdata
          ) {
            console.debug("jwtdata=" + JSON.stringify(jwtdata));
            ...
            conn.sobject("Account").create(
              {
                Name:
                  "My Account #" + Math.floor(Math.random() * Math.floor(100))
              },
              function(err, ret) {
                if (err || !ret.success) {
                  console.log("Account insert error=" + err);
  ...
  app.post("/registeruser", (req, res) => {
    ...
    console.debug(
      "registeruser called req.accessToken=" + req.body.accessToken
    );
    console.debug(
      "registeruser called req.refreshToken=" + req.body.refreshToken
    );
    console.debug(
      "registeruser called req.instance_url=" + req.body.instanceUrl
    );
  ...
  app.get("/oauth/callback", (req, res) => {
    ...
    conn.authorize(authorizationCode, function(err, userInfo) {
      ...
      conn.query(
        "SELECT Id, Name, email, username FROM User where Id='" +
          userInfo.id +
          "'",
        (err, result) => {
